...
export default function ProposalPage({ params }: { params: { proposalId: string } }) {
  const proposal = PROPOSALS[params.proposalId];
        ...
        {proposal.status === 'Active' && !voted ? (
          <Card title="Cast Your Vote">
            <div className="flex flex-col gap-3">
              <Button
                onClick={() => { castVote(BigInt(params.proposalId), 1); setVoted(true); }}
                isLoading={isPending}
                variant="primary"
              >
                Vote For
              ...
              <Button
                onClick={() => { castVote(BigInt(params.proposalId), 0); setVoted(true); }}
                isLoading={isPending}
                variant="danger"
              >
                Vote Against
              ...
              <Button
                onClick={() => { castVote(BigInt(params.proposalId), 2); setVoted(true); }}
                isLoading={isPending}
                variant="secondary"
              >
                Abstain
